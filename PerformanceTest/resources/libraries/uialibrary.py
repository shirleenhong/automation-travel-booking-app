...
class Control(LegacyIAccessiblePattern, QTPLikeSyntaxSupport):
    def __init__(self, element = 0, searchFromControl = None, searchDepth = 0xFFFFFFFF, searchWaitTime = SEARCH_INTERVAL, foundIndex = 1, **searchPorpertyDict):
        ...
        regClassName = searchPorpertyDict.get('RegexClassName', '')
        regAutomationId = searchPorpertyDict.get('RegexAutomationId', '')
        self.regexName = re.compile(regName) if regName else None
        self.regexClassName = re.compile(regClassName) if regClassName else None
        self.regexAutomationId = re.compile(regAutomationId) if regAutomationId else None
    ...
    def AddSearchProperty(self, **searchPorpertyDict):
        ...
        if 'RegexName' in searchPorpertyDict:
            ...
            self.regexName = re.compile(regName) if regName else None
        if 'RegexClassName' in searchPorpertyDict:
            regClassName = searchPorpertyDict['RegexClassName']
            self.regexClassName = re.compile(regClassName) if regClassName else None            
        if 'RegexAutomationId' in searchPorpertyDict:
            regAutomationId = searchPorpertyDict['RegexAutomationId']
            self.regexAutomationId = re.compile(regAutomationId) if regAutomationId else None   
    ...
    def RemoveSearchProperty(self, **searchPorpertyDict):
        for key in searchPorpertyDict:
            ...
            if key == 'RegexName':
                self.regexName = None
            if key == 'RegexClassName':
                self.regexClassName = None
            if key == 'RegexAutomationId':
                self.regexAutomationId = None                
        ...
        for key, value in self.searchPorpertyDict.items():
            ...
            if 'RegexName' == key:
                if not self.regexName.match(control.Name):
                    return False
            if 'RegexClassName' == key:
                if not self.regexClassName.match(control.ClassName):
                    return False
            if 'RegexAutomationId' == key:
                if not self.regexAutomationId.match(control.AutomationId):
                    return False                                       
            if 'Depth' == key:
                if value != depth:
                    return False
    ...
    def Exists(self, maxSearchSeconds = 10, searchIntervalSeconds = SEARCH_INTERVAL):
    ...
    def __unicode__(self):
        ...
        return unic('ControlType: {0}    ClassName: {1}    AutomationId: {2}    Rect: {3}    Name: {4}    Handle: 0x{0:X}({1})'.format(self.ControlTypeName, self.ClassName, self.AutomationId, self.BoundingRectangle, self.Name, self.Handle))
...
class ComboBoxControl(Control, ExpandCollapsePattern, SelectionPattern, ValuePattern):
    ...
    def Select(self, name, waitTime = OPERATION_WAIT_TIME):
        ...
        if not find:
            raise ValueError('Can\'t find {} in ComboBoxControl'.format(name))
