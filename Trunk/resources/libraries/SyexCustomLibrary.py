...
class SyexCustomLibrary:
    ...
    def is_error_icon_visible(self, parent_auto_id):
        ...
        try:
            ...
            return bool(re.search(r'\d', uialibrary.GroupControl(AutomationId=parent_auto_id)\
                            .GetFirstChildControl().AutomationId))
    ...
    def get_test_cases_based_on_keyword_usage(self, keyword, test_folder, project='apac'):
        if project == 'apac':
            source = path.abspath(path.join(__file__, '../../../tpro/acceptance_tests/' + test_folder))
        else:
            source = path.abspath(path.join(__file__, '../../../../Efficiency_18.10/acceptance_tests/' + test_folder))
        suite = TestData(source=source)
        def search_kw_in_test_cases():
            for v in suite.children:
                for t in v.testcase_table.tests:
                    for s in t.steps:
                        steps = '\t'.join((str(s.name), '    '.join(s.args), ''.join(s.comment._comment)))
                        if keyword in steps:
                            yield t.name
        return "Test cases:\n" + "\n".join(set([tc for tc in search_kw_in_test_cases()]))
